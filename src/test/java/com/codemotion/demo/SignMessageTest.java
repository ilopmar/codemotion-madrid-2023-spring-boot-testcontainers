...
import com.codemotion.demo.controller.PlainMessage;
import com.codemotion.demo.controller.SignedMessageDTO;
...
public class SignMessageTest extends AbstractContainerTest {
  ...
  void signText() {
    var response = given(requestSpecification)
        .when().body(new PlainMessage("Hola Codemotion"))
        .post("/messages");
    ...
    await().untilAsserted(() -> {
      ...
      var signature = given(requestSpecification)
          .when()
  ...
  void signSomeTexts() {
    for (int i = 1; i <= 5; i++) {
      given(requestSpecification)
          .when().body(new PlainMessage("Hola Codemotion " + i))
          .post("/messages")
          .then()
    ...
    await().timeout(Duration.ofSeconds(30)).untilAsserted(() -> {
      ...
      assertThat(signedMessages).allMatch(signedMessageDTO -> signedMessageDTO.signature() != null);
    });
  }
